...
class DAG:
    ...
    def toggle_all_dags(
        ...
        """Toggling all DAGs in the given environment."""
        sub_command = f"dags {operation}"
        regex = ".*"  # ignore airflow_monitoring
        all_dags_exp = f'-y --treat-dag-id-as-regex "{regex}"'
        ...
        command = (
            ...
            f" {sub_command} -- {all_dags_exp}"
        )
        command_output = DAG._run_shell_command_locally_once(command=command)
        if command_output[0] == 1:
            logger.info(command_output[1])
            logger.info(f"Error toggling all DAGs. Operation {operation} Retrying...")
            command_output = DAG._run_shell_command_locally_once(command=command)
            if command_output[0] == 1:
                logger.info(f"Unable to toggle all DAGs with operation {operation}")
        logger.info(command_output[1])
    ...
    def toggle_dag(
        ...
        """Toggle specific DAG in the given environment."""
        sub_command = f"{operation}" if airflow_version < (2, 0, 0) else f"dags {operation}"
        command = (
            f"CLOUDSDK_API_ENDPOINT_OVERRIDES_COMPOSER={sdk_endpoint} gcloud composer environments"
            f" run {environment} --project={project_name} --location={location}"
        ...
        if command_output[0] == 1:
            logger.info(command_output[1])
            logger.info(f"Error toggling DAG {dag_id}, with operation {operation} Retrying...")
            command_output = DAG._run_shell_command_locally_once(command=command)
            if command_output[0] == 1:
                logger.info(f"Unable to toggle DAG {dag_id}, with operation {operation}")
        logger.info(command_output[1])
